...
from commands import change_phone, show_all, good_bye, add_phone, change_name, add_bd, get_bd,  search_by_name, search_by_phone
import re
        ...
        if usr_inp in list_cmds:
            ...
            if usr_inp.split()[0] in cmds.keys() and len(usr_inp.split()) > 1:
                ...
                if len(data[0].split(", ")) == 2: # old_name new_name
                    ...
                    continue
                if len(args) >= 2:
                    if all(symb.isdigit() or symb in ['+', '-'] for symb in args[-2]): # add name phone birthday
                        ...
                        continue
                    elif all(symb.isdigit() or symb in ['+', '-'] for symb in args[-1]): # add name phone; change_phone name phone; del_phone name phone;

...
class Record(Field):
    ...
    def days_to_birthday(self, birthday):
        ...
        delta = next_bd - today
        return f"Дней до дня рождения: {delta.days}"
...
class AddressBook(UserDict):
    ...
    def find_records(self, name=None, phone=None, birthday=None):
        if name is not None:
            return self.data.get(name.value, [])
        elif phone is not None:
            return [record for record in self.data.values() if phone.value in record.phones]
        elif birthday is not None:
            return [record for record in self.data.values() if record.birthday == birthday.value]
        else:

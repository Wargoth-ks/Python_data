    ...
    while True:
        ...
        if usr_inp in list_cmds:
            ...
            if usr_inp.split()[0] in cmds.keys() and len(usr_inp.split()) > 1:
                ...
                date_arg = re.match(r'^(0[1-9]|1[0-9]|2[0-9]|3[01])\.(0[1-9]|1[0-2])\.\d{4}$',args[-1])
                #print(command, args)
                if not data:
                    print(error_func())
                    continue
                if len(data[0].split(", ")) == 2: # old_name new_name
                    ...
                    continue
                if len(args) >= 2:
                    if re.match(r'^\+38\d{10}$', args[-2]) and date_arg: # add name phone birthday
                        ...
                        continue
                    elif re.match(r'^\+38\d{10}$', args[-1]): # add name phone; change_phone name phone; del_phone name phone;
                        dig_arg = str(args[-1]) # phone
                        list_args = [" ".join(args[:-1]), dig_arg]
                        print(handler(*list_args))
                        continue
                if len(args) >= 1 and date_arg is None: # by_phone, by_name
                    #print(data)
                    print(handler(*data))
                    continue
                if date_arg:

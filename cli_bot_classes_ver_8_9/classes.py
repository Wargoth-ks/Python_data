...
class Name(Field):
    ...
    def value(self, value):
        if not re.match(r'^[A-Za-z\s]+$', value):
            raise ValueError(f"The name should not contain digits or special characters!!!")
...
class Phone(Field):
    ...
    def value(self, value):
        if not re.match(r'^\+38\d{10}$', value):
            raise ValueError("Format of number should be +380XXXXXXXXX !!!")
        Field.value.fset(self, value)
...
class Record(Field):
    ...
    def delete_phone(self, phone: Phone):
        if phone not in self.phones:
            raise ValueError(f"<< {self.name.value} >> doesn't have << phone: {phone} >>")
        self.phones.remove(phone)
...
class AddressBook(UserDict):
    ...
    def find_records(self, name=None, phone=None):
        if name is not None:
            ...
            records = [str(rec) for rec in self.data.values() 
                if any(word.lower().startswith(patt_name[0].lower()) 
                for word in rec.name.value.split())] if patt_name else []
            str_rec = "\n\n".join(records)
            return str_rec
            ...
            for record in self.data.values():
                for p in record.phones:
                    matches = re.findall(r"\d{3,}", p.value)
    ...
    def print_records(self, records):
        for record in records:
            ...
            if info['birthday']:
                print(colored("Birthday: ", "yellow") + f"{info['birthday'].value.strftime('%d.%m.%Y')}")
            else:

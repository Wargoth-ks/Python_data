...
def add_contact(name, phone, birthday=None):
    name_obj = Name(name)
    phone = Phone(phone)
    if name_obj.value not in contacts:
        if birthday is not None:
            birthday = Birthday(birthday)
            record = Record(name=name_obj, phones=[phone], birthday=birthday)
        else:
            record = Record(name=name_obj, phones=[phone])
        contacts.add_record(record)

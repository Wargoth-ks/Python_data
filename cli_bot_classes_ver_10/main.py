...
cmds = {"add": add_contact, # >= 2 arg name phone (bd) +
...
def main():
    while True:
        ...
        if usr_inp in list_cmds.keys():
            s_func = list_cmds[usr_inp.strip()]
            print(s_func())
            ...
            if usr_inp.split()[0] in cmds.keys() and len(usr_inp.split()) > 1:
                ...
                args = data[0].split(" ")
                print(f"From main: {command} {args}")
                print(handler(*args))
